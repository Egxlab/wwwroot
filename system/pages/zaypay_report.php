...
require_once(SYSTEM . 'pay/zaypay/config.php');
require_once(SYSTEM . 'pay/zaypay/zaypay.php');
...
if(isset($_GET['payment_id']) && isset($_GET["price_setting_id"]) && isset($options[$_GET["price_setting_id"]]))
{
	...
	if($zaypay_info["payment"]["status"] == 'paid' && !file_exists(SYSTEM . 'pay/zaypay/used_codes/zay_id_' . $zaypay_info["payment"]["id"]. '.txt'))
	{
		$_acc = explode('=', $zaypay_info["payment"]["your-variables"]);
		$acc = $_acc[1];
		if(file_put_contents(SYSTEM . 'pay/zaypay/used_codes/zay_id_' . $zaypay_info["payment"]["id"]. '.txt' , 'account:' . $acc . ',amount-euro:' . $zaypay_info["payment"]["amount-euro"] . ',points:' . $option['points']) !== false)
		{
