 ...
 */
	...
	public function verifySpin($account_id)
	{
         $data = $this->db->Query("SELECT count(*)cnt FROM Flankera_gifts where spin_date=CURDATE() and account_id=$account_id")->fetch();
		 if($data["cnt"]==0){return true;}else{return false;}
	}
	
	public function sendGift($account_id, OTS_Item $item)
	{
		  
		  $item_id = $item->getId();
		  $count = $item->getCount();
		  $this->db->Query("INSERT INTO flankera_gifts(item_id,account_id,count,spin_date) values($item_id,$account_id,$count,CURDATE()) ");
	}
